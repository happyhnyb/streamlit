...
[data-testid="stMetricLabel"] { font-family: 'JetBrains Mono', monospace !important; font-size: 0.7rem !important; color: var(--muted) !important; text-transform: uppercase; letter-spacing: 0.1em; }
[data-testid="stMetricValue"] { font-family: 'Orbitron', monospace !important; font-size: 1.4rem !important; color: var(--gold) !important; }
...
def generate_f1_data():
    ...
    # Championship standings per season
    ...
    for season in [2020, 2021, 2022, 2023, 2024]:
        for d_code, d_name, team, _, _ in drivers[:5]:
            for lap in range(1, 55):
                ...
                # slight improvement over laps (fuel burn)
                fuel_delta = -0.03 * lap
                lap_rows.append({
                    "season": season,
                    "driver_code": d_code,
    ...
    with col1:
        ...
        if st.button("▶  RUN INGESTION PIPELINE"):
            steps = [
                ...
                ("Downloading races.csv (150K rows)...", 0.5),
                ("Downloading drivers.csv (2,340 rows)...", 0.3),
                ("Downloading lap_times.csv (4.28M rows)...", 0.8),
                ("Downloading results.csv (520K rows)...", 0.5),
                ("Schema validation — races.csv...", 0.3),
                ("Schema validation — lap_times.csv...", 0.3),
                ("Writing to raw landing zone (S3)...", 0.4),
                ("Registering metadata in catalog...", 0.3),
                ("Triggering downstream Spark job...", 0.3),
            ]
            log_box = st.empty()
            prog = st.progress(0)
            logs = []
            for i, (msg, delay) in enumerate(steps):
                ...
                log_html = "<div style='background:#0A0A0F;border:1px solid #2A2A38;border-radius:4px;padding:0.75rem 1rem;height:220px;overflow-y:auto;'>" + "<br>".join(logs) + '<br><span class="log-ok">✓ Pipeline complete — 5.44M rows ingested in 6m 32s</span>' * (1 if i == len(steps)-1 else 0) + "</div>"
...
def transform_results(spark) -> None:
    ...
    # Normalize points (handle sprint/penalty edge cases)
    point_expr = when(col("position").isin(list(POINTS_MAP)), 
                      col("points")).otherwise(lit(0))

    # Partition window for season ranks
...
def load_to_snowflake(df, table: str, schema: str = "ANALYTICS") -> None:
    ...
    # MERGE strategy for idempotent loads
...
"""
    ...
    with tab3:
        ...
        for name, vals, color in [("CPU %", cpu, "#E10600"), ("Memory %", mem, "#3A8DFF"), ("Network MB/s (scaled)", net, "#FFC906")]:
            fig.add_trace(go.Scatter(x=mins, y=vals, name=name, line=dict(color=color, width=2), fill="tozeroy", fillcolor=color.replace("#","#22")+"33"))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        fig = go.Figure(go.Bar(
            x=top10["driver_code"], y=top10["points"],
            marker_color=[COLORS[i % len(COLORS)] for i in range(len(top10))],
            text=top10["points"], textposition="outside",
            textfont=dict(family="JetBrains Mono", size=10, color="#E8E8F0")
        ))
        fig.update_layout(**LAYOUT, title=f"{season_filter} Championship — Top 10 Drivers",
    ...
    with col2:
        ...
        fig2 = go.Figure(go.Pie(
            labels=team_pts.index, values=team_pts.values,
            hole=0.55,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        fig = go.Figure(go.Bar(
            ...
            marker_color=COLORS[:8], text=career["Wins"][:8],
    ...
    with col2:
        fig2 = go.Figure(go.Bar(
            ...
            marker_color=[c + "AA" for c in COLORS[:8]], text=career["Podiums"][:8],
            textposition="outside",
            textfont=dict(family="JetBrains Mono", size=10, color="#E8E8F0")
        ))
    ...
    for i, row in career.head(3).iterrows():
        vals = [row["Wins"]/career["Wins"].max()*100,
                row["Podiums"]/career["Podiums"].max()*100,
                row["Points"]/career["Points"].max()*100,
                row["Races"]/career["Races"].max()*100,
                row["Win %"]/career["Win %"].max()*100]
        vals += [vals[0]]
        fig3.add_trace(go.Scatterpolar(
            r=vals, theta=metrics+[metrics[0]],
            name=row["Code"], fill="toself",
            line=dict(color=COLORS[list(career.index).index(i) % len(COLORS)], width=2),
            opacity=0.6,
        ))
    fig3.update_layout(**LAYOUT, polar=dict(
        bgcolor="#0A0A0F",
    ...
    ), title="Normalised Performance Index", height=350)
    st.plotly_chart(fig3, use_container_width=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if len(con_sel):
        ...
        with c4: st.metric("Points Gap",    int(con_sel.iloc[0]["points"] - con_sel.iloc[1]["points"]) if len(con_sel)>1 else "—")

    col1, col2 = st.columns([2, 3])
    with col1:
        fig = go.Figure(go.Bar(
            x=con_sel["points"].head(8), y=con_sel["team"].head(8),
            orientation="h",
            marker_color=COLORS[:8],
            text=con_sel["points"].head(8), textposition="outside",
            textfont=dict(family="JetBrains Mono", size=10, color="#E8E8F0"),
        ))
        fig.update_layout(**LAYOUT, title=f"{season_filter} Constructor Standings",
                          height=320, showlegend=False, xaxis_title="Points", yaxis=dict(autorange="reversed",**LAYOUT["yaxis"]))
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        # Multi-season constructor trend
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        fig.update_layout(**LAYOUT, title="Lap Time Progression", height=290,
    ...
    with col2:
        ...
        for i, d in enumerate(top_drivers):
            ...
            dist_data.append(go.Violin(y=dd, name=d, line_color=COLORS[i], fillcolor=COLORS[i]+"44", box_visible=True, meanline_visible=True))
    ...
    for j, sec in enumerate(["sector1","sector2","sector3"]):
        fig3.add_trace(go.Bar(
            x=sector_agg["driver_code"], y=sector_agg[sec],
            name=f"S{j+1}", marker_color=COLORS[j]
        ))
    fig3.update_layout(**LAYOUT, barmode="stack", title="Sector Time Composition",
                       height=280, xaxis_title="Driver", yaxis_title="Seconds")
    st.plotly_chart(fig3, use_container_width=True)
    ...
    fig4 = go.Figure(go.Bar(
        x=consistency["Driver"], y=consistency["Std Dev (s)"],
        marker_color=[COLORS[i % len(COLORS)] for i in range(len(consistency))],
        text=consistency["Std Dev (s)"].round(3), textposition="outside",
        textfont=dict(family="JetBrains Mono", size=10, color="#E8E8F0")
    ))
    fig4.update_layout(**LAYOUT, title="Lap Time Std Deviation", height=250, showlegend=False,
                       xaxis_title="Driver", yaxis_title="Std Dev (s)")
